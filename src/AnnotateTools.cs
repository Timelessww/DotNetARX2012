...
/// 注释缩放操作类
/// </summary>
public static partial class AnnotateTools
{
#if !NET35
    /// <summary>
    /// 为实体添加指定的注释比例
    ...
    public static List<ObjectContext> GetAllScales(this ObjectId entId)
    {
        ...
        List<ObjectContext> scales = new();
        var obj = entId.GetObject(OpenMode.ForRead);
        if (obj.Annotative != AnnotativeStates.NotApplicable)
        {
            var ocm = obj.Database.ObjectContextManager;
            var occ = ocm.GetContextCollection("ACDB_ANNOTATIONSCALES");
            // 遍历注释比例列表
    ...
    public static AnnotationScale? AddScale(this Database db, string scaleName, double paperUnits, double drawingUnits)
    {
        ...
        if (!occ.HasContext(scaleName))
        {
            ...
            {
                Name = scaleName,           // 注释比例名
                PaperUnits = paperUnits,          // 注释比例的图纸单位
                DrawingUnits = drawingUnits         // 注释比例的图形单位
            };                                      // 新建一个注释比例对象
