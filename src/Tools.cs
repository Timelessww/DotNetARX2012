...
public static partial class Tools
{
    ...
    public static ObjectIdCollection AddToPaperSpace(this Database db, params Entity[] ents)
    {
        ...
        var trans = db.TransactionManager;
        var btr = (BlockTableRecord)trans.GetObject(SymbolUtilityServices.GetBlockPaperSpaceId(db), OpenMode.ForWrite);
        foreach (var ent in ents)
        {
            ids.Add(btr.AppendEntity(ent));
            trans.AddNewlyCreatedDBObject(ent, true);
    ...
    public static void HighlightEntities(this SelectionSet selectionSet)
    {
        ...
        var ids = new ObjectIdCollection(selectionSet.GetObjectIds());
    ...
    {
        var strPoint = stringPoint.Trim().Split(new char[] { '(', ',', ')' }, StringSplitOptions.RemoveEmptyEntries);
    ...
    {
        return Acap.DocumentManager.GetDocument(db);
    }
    ...
    {
        return Acap.DocumentManager.GetDocument(db).Editor;
    }
