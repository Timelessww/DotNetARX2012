...
public static class EntTools
{
    ...
    public static ObjectIdCollection Offset(this ObjectId id, double dis)
    {
        ...
        var cur = id.GetObject(OpenMode.ForWrite) as Curve;
        if (cur == null)
    ...
    public static DBObjectCollection Offset(this Curve cur, double dis)
    {
        var offsetCurves = new DBObjectCollection();
        try
        {
    ...
    public static ObjectIdCollection? ArrayRectang(this ObjectId id, int numRows, int numCols, double disRows, double disCols)
    {
        ...
        for (int m = 0; m < numRows; m++)
        {
            ...
            {
                // 获取平移矩阵
                var mt = Matrix3d.Displacement(new Vector3d(n * disCols, m * disRows, 0));
                var entCopy = ent.GetTransformedCopy(mt);// 复制实体
                // 将复制的实体添加到模型空间
                var entCopyId = id.Database.AddToModelSpace(entCopy);
    ...
    {
        Extents3d totalExt = new();
        using (var trans = db.TransactionManager.StartTransaction())
        {
            var bt = (BlockTable)trans.GetObject(db.BlockTableId, OpenMode.ForRead);
            var btRcd = (BlockTableRecord)trans.GetObject(bt[BlockTableRecord.ModelSpace], OpenMode.ForRead);
            foreach (var entId in btRcd)
            {
