...
#pragma warning disable CA2101 // 指定对 P/Invoke 字符串参数进行封送处理
    ...
    [DllImport("kernel32.dll", CharSet = CharSet.Auto)]
    static extern IntPtr GetProcAddress(IntPtr dllPointer, string functionName);
    ...
    static extern int acedGetEnv(string envName, StringBuilder result);
...
#pragma warning disable IDE0060 // 删除未使用的参数
    ...
    {
        StringBuilder ret = new(1024);
        _ = acedGetEnv(symbol, ret);
        return ret.ToString();
    }

    [DllImport("acad.exe", CharSet = CharSet.Auto, CallingConvention = CallingConvention.Cdecl)]
    static extern int acedSetEnv(string envName, StringBuilder value);
    /// <summary>
    /// 设置环境变量值
    /// </summary>
    /// <param name="ed">命令行对象,无意义,只是为了创建扩展函数</param>
    ...
    public static Bitmap? GetPreviewBitmapFromDwg(this Database db, string fileName)
    {
        ...
        var delegateFunc = (acdbGetPreviewBitmapFromDwg)GetDelegateForFunction(true, func32, func64, typeof(acdbGetPreviewBitmapFromDwg))!;
    ...
    public static void AddPersistentReactor(this DBObject dbObject, ObjectId objId)
    {
        ...
        var delegateFunc = (addPersistentReactor)GetDelegateForFunction(true, func, func, typeof(addPersistentReactor))!;
    ...
    static extern IntPtr acedGenerateThumbnailBitmap();
    /// <summary>
    /// 为当前图形生成缩略图
    /// </summary>
    /// <param name="ed">命令行对象,无意义,只是为了创建扩展函数</param>
...
#pragma warning restore IDE0060 // 删除未使用的参数
    ...
    public static bool AnnotativeDwg(this Database db)
    {
        ...
        var delegateFunc = (annotativeDwg)GetDelegateForFunction(true, func32, func64, typeof(annotativeDwg))!;
